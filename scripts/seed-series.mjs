...
const catalogue = [
  {
    ...
    note: 4.8,
    nombre_avis: 16,
    nombre_commandes_semaine: 11,
    sort_order: 5,
    avis: [
      { nom: "Clarisse N.", note: 5, commentaire: "Ma fille a tout de suite voulu savoir ce qui arrivait aux animaux. Quand elle a vu son prénom dans l'histoire, elle ne voulait plus lâcher la BD.", ville: "Douala", date: "il y a 3 jours", avatar: "C" },
      { nom: "Marius E.", note: 5, commentaire: "Très belle idée de cadeau. Mon fils adore les lions, et la commande sur WhatsApp était simple.", ville: "Yaoundé", date: "il y a 6 jours", avatar: "M" },
      { nom: "Aïcha T.", note: 4, commentaire: "Les dessins sont beaux et l'histoire tient bien l'attention des enfants. Très bon retour après la livraison.", ville: "Bafoussam", date: "il y a 2 semaines", avatar: "A" },
    ],
  ...
  const { error } = await supabase.from("series").upsert(
    {
      ...
      disponible: true,
      landing_page_mode: item.landing_page_mode ?? false,
      published: true,
      archived_at: null,
      note: item.note,
