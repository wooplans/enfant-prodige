...
import { FACEBOOK_PIXEL_ID, WHATSAPP_ORDER_EVENT_NAME } from "@/lib/facebook";
...
export type FacebookWhatsAppOrderEventInput = {
  eventSourceUrl: string;
  serie: string;
  prenom: string;
...
export async function sendFacebookWhatsAppOrderEvent(input: FacebookWhatsAppOrderEventInput) {
  const token = process.env.FACEBOOK_CONVERSION_API_TOKEN?.trim();
  if (!token) {
    throw new Error("FACEBOOK_CONVERSION_API_TOKEN manquant.");
  }
  ...
  const response = await fetch(`https://graph.facebook.com/${version}/${FACEBOOK_PIXEL_ID}/events?access_token=${encodeURIComponent(token)}`, {
    ...
    body: JSON.stringify({
      data: [
        {
          event_name: WHATSAPP_ORDER_EVENT_NAME,
          event_time: eventTime,
